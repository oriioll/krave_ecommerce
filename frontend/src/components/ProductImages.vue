...
const goToImage = (dir: string) => {
    ...
    if (dir == 'left') {
        ...
        const newImgNumber = Number.parseInt(currentImg) - 1 >= 0 ? Number.parseInt(currentImg) - 1 : 3;
        window.location.href = baseUrl + '#image-' + newImgNumber;
    } else {
        //If the current image - 1 is bigger then 0 (current image is 1, 2...) Substract one, if its 0 or less, new image is still 0
        const newImgNumber = Number.parseInt(currentImg) + 1 <= 3 ? Number.parseInt(currentImg) + 1 : 0;
        window.location.href = baseUrl + '#image-' + newImgNumber;
    }
}
</script>
    ...
    <aside class="container">
        <div class="slider-wrapper">
            <div class="slider">
                ...
                <img v-for="(image, index) in images" :src="image!" alt="product" :id="'image-' + index">
